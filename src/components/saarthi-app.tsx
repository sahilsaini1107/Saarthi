...
function BottomNav({ active, onNavigate }: { active: string; onNavigate: (to: string) => void }) {
  return (
    <nav
      ...
      className="fixed bottom-0 left-1/2 z-40 w-full max-w-[480px] -translate-x-1/2 border-t bg-background/95 pb-[env(safe-area-inset-bottom)] backdrop-blur print:hidden"
    >
      <div className="grid grid-cols-5">
        {TABS.map((tab) => {
          const isActive = active === tab.path || (tab.path !== '/today' && active.startsWith(tab.path))
          const Icon = tab.icon
          return (
            <button
              key={tab.path}
...
function renderScreen(path: string, navigate: (to: string) => void, user: UserDTO): ReactNode {
  ...
  if (path.startsWith('/money/accounts/')) return <AccountDetailScreen accountId={path.split('/')[3]} />
...
function Shell({ user, path, navigate }: { user: UserDTO; path: string; navigate: (to: string) => void }) {
  const [quickAdd, setQuickAdd] = useState<{ open: boolean; editTxn?: TransactionDTO; tripId?: string }>({ open: false })
  ...
  const openQuickAdd = useCallback((editTxn?: TransactionDTO, opts?: { tripId?: string }) => setQuickAdd({ open: true, editTxn, tripId: opts?.tripId }), [])
    ...
    <UiContext.Provider value={{ user, navigate, openQuickAdd }}>
      <div className="relative mx-auto flex min-h-dvh w-full max-w-[480px] flex-col bg-background print:max-w-none">
        ...
        <QuickAddSheet
          ...
          onOpenChange={(o) => (o ? openQuickAdd(quickAdd.editTxn, { tripId: quickAdd.tripId }) : closeQuickAdd())}
        />
      </div>
    </UiContext.Provider>
  )
...
function OfflineBanner() {
  ...
  return (
    <div className="sticky top-0 z-50 bg-warn/15 px-4 py-2 text-center text-xs font-medium text-warn backdrop-blur" role="status">
