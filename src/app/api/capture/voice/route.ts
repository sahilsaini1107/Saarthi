...
import { parseVoiceCapture, suggestCategoryId } from '@/services/capture'
import { db } from '@/lib/db'
import { checkCaptureRateLimit, pruneRateLimitStores } from '@/lib/rate-limit'

const schema = z.object({
  /** base64-encoded 16 kHz mono WAV (encoded client-side, lib/wav.ts) */
...
export async function POST(req: Request) {
  return withUser(async (user) => {
    ...
    const result = await parseVoiceCapture(user.timezone, { audioBase64: body.audioBase64 })
